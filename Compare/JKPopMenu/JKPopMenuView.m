...
//        maskView.backgroundColor = [RGB_COLOR(210, 210, 210) colorWithAlphaComponent:1];
...
- (void)setupSubviews
{
    ...
    if (count == 2 || count == 4) {
        ...
        for (JKPopMenuItem *item in self.menuItems) {
            ...
            item.frame = CGRectMake(offset + (PopMenuItemWidth+offset)*x_index, yStart + (PopMenuItemHeight + y_Offset)*y_index + (JK_SCREEN_HEIGHT - firstItemY)-fixHeight, PopMenuItemWidth, PopMenuItemHeight);
            switch (x_index) {
                case 0:
